...
class AirportDestinationSyncer:
    ...
    def sync_all_destinations(self, taiwan_airports_only=True):
        """同步所有機場的目的地數據"""
        with self.app.app_context():
            print(f"[{datetime.now()}] 開始同步機場目的地數據...")
            
            # 獲取需要同步的出發機場
            if taiwan_airports_only:
                departure_airports = Airport.query.filter_by(country='Taiwan').all()
                print(f"找到 {len(departure_airports)} 個台灣機場需要同步")
            else:
                departure_airports = Airport.query.all()
                print(f"找到 {len(departure_airports)} 個機場需要同步")
            ...
            for airport in departure_airports:
                try:
                    ...
                    destinations = self._sync_single_airport_destinations(airport.airport_id)
...
def main():
    ...
    # 同步台灣機場的目的地
    syncer.sync_all_destinations(taiwan_airports_only=True)
